...
Compile: Take an OpenAPI/GraphQL/proto spec and compile it into a small list of endpoint signatures & constraints.

Plan: The LLM emits one CAL JSON for the user’s goal (deterministic JSON-only output).

Execute: The runtime turns CAL into real HTTP/library calls.

Validate & Repair: If the call fails or violates constraints, a critic proposes a corrected CAL (guided by the spec + error text). Try again.
...
If your chosen model is gated on Hugging Face, create a token at https://huggingface.co/settings/tokens
 and pass it with --hf-token or set export HF_TOKEN=....
